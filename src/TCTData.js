...
class TCTData {
    ...
    getAdvisorFeedbackForAnswer(pk, playableCandidatePk) {
        return Object.values(this.answer_feedback).filter(feedback => feedback.fields.answer == pk && feedback.fields.candidate == playableCandidatePk);
    }
    ...
    answersAsOseg(question, playableCandidatePk) {
        const answers = this.getAnswersForQuestion(question.pk);
        return answers.map((answer) => {

            const fed = this.getAdvisorFeedbackForAnswer(answer.pk, playableCandidatePk);
    ...
    questionsAsOseg(playableCandidatePk) {
        ...
        return questions.map((question) => {
            return {
                ...
                answers: this.answersAsOseg(question, playableCandidatePk),
    ...
    exportToOseg(playableCandidatePk) {
        ...
        const oseg = {
            ...
            scenarioSides: [
                {
                    playerId: playableCandidatePk,
                    questions : this.questionsAsOseg(playableCandidatePk)
                }
            ],
...
function loadDataFromFile(raw_json) {
    ...
    const data = new TCTData(questions, answers, issues, state_issue_scores, candidate_issue_scores, running_mate_issue_scores, candidate_state_multipliers, answer_score_globals, answer_score_issues, answer_score_states, feedbacks, states, highest_pk, jet_data)
...
export {TCTData, loadDataFromFile}